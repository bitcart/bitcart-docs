...
Due to it's lightness, you can host BitcartCC on a minimal server, with minimal costs! Current lunanode 1-click installer would cost you **3.5$ a month** to host BitcartCC with all components\(admin, store\) and from 1 to 5 cryptocurrencies for the same price, still being light. Or you can find even cheaper hosting providers and host it even cheaper!  In fact, all BitcartCC demos\([store](https://store.bitcartcc.com), [admin](https://admin.bitcartcc.com), [api](https://demo.bitcartcc.com)\) run on 1 GB server with only 25 GB disk.
